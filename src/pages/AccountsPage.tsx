...
type MenuState = { accountId: string; x: number; y: number } | null;
...
export default function AccountsPage({ user, selectedAccountId, onSelectAccount, onTransfer, onNewAccount, refreshKey, onRefresh }: AccountsPageProps) {
  ...
  const fetchAll = useCallback(async (silent = false) => {
    try {
      ...
      if (active) {
        ...
        const newBalance = active.balance;
        const newCount = txs.length;
        if (prevDataRef.current) {
          if (prevDataRef.current.balance !== newBalance || prevDataRef.current.txCount !== newCount) {
            setTransactions(txs);
  ...
  const handleLongPress = (e: React.MouseEvent | React.TouchEvent, accountId: string) => {
    e.preventDefault();
    hapticImpact('medium');
    const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
    setMenu({ accountId, x: rect.left, y: rect.bottom + 4 });
  };
  ...
  return (
    <div className="px-4 pt-2 pb-24 animate-fade-in" onClick={() => menu && setMenu(null)}>
        ...
        {accounts.map((acc) => {
          ...
          return (
            <button
              ...
              onContextMenu={(e) => handleLongPress(e, acc.id)}
              onTouchStart={(e) => {
                const timer = setTimeout(() => handleLongPress(e, acc.id), 500);
                const cancel = () => clearTimeout(timer);
                e.currentTarget.addEventListener('touchend', cancel, { once: true });
                e.currentTarget.addEventListener('touchmove', cancel, { once: true });
              }}
              className={`px-4 py-2 rounded-full whitespace-nowrap text-sm font-medium transition-all duration-200 flex-shrink-0 ${isActive ? 'text-white shadow-md' : 'bg-surface text-on-surface-variant border border-outline/50'}`}
      ...
      {menu && (
        <div
          className="fixed z-50 bg-surface border border-outline/20 rounded-2xl shadow-xl overflow-hidden"
          style={{ top: menu.y, left: 16, right: 16 }}
      ...
      {editState && (
        <div className="fixed inset-0 z-50 flex items-end bg-on-surface/30">
          <div className="bg-surface rounded-t-3xl p-6 w-full shadow-2xl animate-slide-up">
              ...
              {ACCOUNT_COLORS.map((c) => (
                <button key={c.hex} onClick={() => setEditState({ ...editState, color: c.hex })} className="flex flex-col items-center gap-1.5">
                  <div
                    ...
                    style={{ background: c.hex, boxShadow: editState.color === c.hex ? `0 0 0 2px var(--color-bg), 0 0 0 4px ${c.hex}` : 'none', transform: editState.color === c.hex ? 'scale(1.1)' : 'scale(1)' }}
                  >
      ...
      {activeAccount && (
        <div className="rounded-[28px] p-6 text-white shadow-xl mb-6" style={{ background: cardBg, boxShadow: `0 12px 40px ${activeAccount.color}40` }}>
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-medium text-white/70">{activeAccount.name}</span>
            <button onClick={() => { hapticImpact('light'); const acc = activeAccount; setMenu({ accountId: acc.id, x: 0, y: 0 }); }} className="w-8 h-8 bg-white/15 rounded-xl flex items-center justify-center">
        ...
        {transactions.length === 0 ? (
          ...
          <div className="space-y-2">
            {transactions.map((tx) => {
              ...
              return (
                <div key={tx.id} className="bg-surface rounded-2xl p-4 flex items-center gap-3 border border-outline/10 shadow-sm">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-on-surface-variant mt-0.5">{formatDate(tx.createdAt)}</p>
                  </div>
