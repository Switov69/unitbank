...
export default function TransferPage({ userId, selectedAccountId, onClose }: TransferPageProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (!trimAccount) {
      setError('Введите счёт получателя');
    ...
    if (!exists) {
      setError('Счёт получателя не найден');
      hapticNotification('error');
      return;
    }
    ...
    return (
      <div className="min-h-full flex flex-col bg-bg animate-fade-in">
        ...
        <div className="flex-1 px-4 pt-6">
          <div className="bg-white rounded-3xl p-6 border border-outline/20 space-y-4">
          ...
          <button
            ...
            disabled={loading}
            className="w-full mt-6 bg-primary text-white py-4 rounded-2xl font-semibold text-base
      ...
      <div className="flex-1 px-4 pt-4 space-y-4">
        ...
        <div>
          ...
          <input
            ...
            value={toAccount}
            onChange={(e) => {
              setToAccount(e.target.value.toLowerCase());
              setError('');
            }}
            placeholder="Например: ub-main или main"
            className="w-full bg-white border-2 border-outline/50 rounded-2xl px-4 py-3.5 text-base
              text-on-surface placeholder:text-on-surface-variant/50
              focus:border-primary focus:outline-none transition-colors"
          />
          <p className="text-xs text-on-surface-variant mt-1.5">
            Введите полное имя счёта (например: ub-main или просто main)
          </p>
        </div>
          ...
          {fromAccount && (
            <div className="flex justify-between mt-2">
              <p className="text-xs text-on-surface-variant">
                Доступно:{' '}
                <span className={`font-medium ${incognitoMode ? 'blur-sm select-none' : ''}`}>
                  {formatAmount(fromAccount.balance)} CBC
                </span>
              </p>
              <button
        ...
        <button
          ...
          className="w-full bg-primary text-white py-4 rounded-2xl font-semibold text-base
            shadow-lg shadow-primary/30 active:scale-[0.98] transition-transform mt-2 disabled:opacity-60"
        >
