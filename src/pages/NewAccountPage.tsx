...
export default function NewAccountPage({ userId, onClose, onCreated }: NewAccountPageProps) {
  ...
  useEffect(() => {
    getAccounts(userId).then((accs) => {
      if (accs.length >= 2) setAtLimit(true);
    });
  }, [userId]);
  ...
  const handleCreate = async () => {
    ...
    if (exists) {
      setError('Такой счёт уже существует');
      hapticNotification('error');
      setLoading(false);
      return;
    }
      ...
      <div className="min-h-full flex flex-col bg-bg animate-slide-up">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6">
          ...
          <p className="text-on-surface-variant text-sm text-center">
            Максимальное количество счетов — 2. Удалите один из существующих, чтобы создать новый.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-full flex flex-col bg-bg animate-slide-up">
      ...
      <div className="flex-1 px-4 pt-4 space-y-5">
        <div
          className="rounded-[24px] p-5 text-white flex items-center gap-4 transition-all duration-300"
