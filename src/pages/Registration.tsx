...
export default function Registration({ onComplete }: RegistrationProps) {
  ...
  const handleNicknameSubmit = async () => {
    ...
    if (trimmed.length < 3) { setFieldError('Минимум 3 символа'); hapticNotification('error'); return; }
    if (trimmed.length > 16) { setFieldError('Максимум 16 символов'); hapticNotification('error'); return; }
    if (!/^[a-zA-Z0-9_]+$/.test(trimmed)) { setFieldError('Только латинские буквы, цифры и _'); hapticNotification('error'); return; }
    setLoading(true);
    const exists = await checkNicknameExists(trimmed);
    setLoading(false);
    if (exists) { setFieldError('Этот никнейм уже занят'); hapticNotification('error'); return; }
    goNext('account');
  };

  const handleAccountSubmit = async () => {
    const trimmed = accountName.trim().toLowerCase();
    if (trimmed.length < 1) { setFieldError('Введите название счёта'); hapticNotification('error'); return; }
    if (trimmed.length > 20) { setFieldError('Максимум 20 символов'); hapticNotification('error'); return; }
    if (!/^[a-zA-Z0-9_-]+$/.test(trimmed)) { setFieldError('Только латинские буквы, цифры, - и _'); hapticNotification('error'); return; }
    const fullName = `ub-${trimmed}`;
    setLoading(true);
    const exists = await checkAccountExists(fullName);
    setLoading(false);
    if (exists) { setFieldError('Такой счёт уже существует'); hapticNotification('error'); return; }
  ...
  const handlePinConfirm = async (confirmedPin: string) => {
    if (confirmedPin !== pin) { setPinError('Коды не совпадают · попробуйте снова'); return; }
    setLoading(true);
    try {
      const tgUser = getTelegramUser();
      const user = await createUser(nickname.trim(), pin, tgUser?.id ?? null, tgUser?.firstName ?? '');
      const account = await createAccount(user.id, `ub-${accountName}`, selectedColor);
      hapticNotification('success');
      setStep('done');
      setTimeout(() => onComplete(user, account.id), 1500);
    } catch (e: unknown) {
      ...
      <div className="flex flex-col min-h-full px-6 pt-16 animate-fade-in">
        ...
        <div className="relative">
          <input
            type="text"
            value={nickname}
            onChange={(e) => { setNickname(e.target.value); setFieldError(''); }}
          ...
          />
          {nickname.length >= 3 && (
            <img src={getAvatarUrl(nickname.trim(), 40)} alt="" className="absolute right-4 top-1/2 -translate-y-1/2 w-8 h-8 rounded-lg" />
          )}
        </div>
        {fieldError && <p className="text-error text-sm mt-2 animate-fade-in">{fieldError}</p>}
        <button
  ...
  if (step === 'account') {
    return (
      <div className="flex flex-col min-h-full px-6 pt-16 animate-fade-in">
        ...
        <p className="text-on-surface-variant text-sm mb-8">Придумайте название для вашего первого счёта</p>
        <div className="flex items-center bg-surface border-2 border-outline/50 rounded-2xl overflow-hidden focus-within:border-primary transition-colors">
          ...
          <input
            type="text"
            value={accountName}
            onChange={(e) => { setAccountName(e.target.value.toLowerCase()); setFieldError(''); }}
            placeholder="main"
            autoFocus
            maxLength={20}
        ...
        <p className="text-on-surface-variant/60 text-xs mt-3">
          Полное имя: <span className="font-medium text-on-surface-variant">ub-{accountName || '...'}</span>
        </p>
        <button
  ...
  if (step === 'color') {
    return (
      <div className="flex flex-col min-h-full px-6 pt-16 animate-fade-in">
        <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-colors duration-300" style={{ background: selectedColor }}>
          <CreditCard className="w-7 h-7 text-white" />
        </div>
        <h1 className="text-2xl font-bold text-on-surface mb-2">Цвет карточки</h1>
        <p className="text-on-surface-variant text-sm mb-8">Выберите цвет для вашего счёта</p>
        <div className="grid grid-cols-4 gap-3 mb-8">
          {ACCOUNT_COLORS.map((c) => (
            <button key={c.hex} onClick={() => { setSelectedColor(c.hex); hapticImpact('light'); }} className="flex flex-col items-center gap-2">
              <div
                className="w-14 h-14 rounded-2xl transition-all duration-200 flex items-center justify-center"
                style={{
                  background: c.hex,
                  boxShadow: selectedColor === c.hex ? `0 0 0 3px var(--color-bg), 0 0 0 5px ${c.hex}` : 'none',
  ...
  if (step === 'pin') {
    return (
      <div className="min-h-full animate-fade-in">
        <PinPad title="Придумайте код-пароль" subtitle="4 цифры для быстрого входа" pinLength={4} onComplete={handlePinSet} />
      </div>
    );
  }
  ...
  if (step === 'confirm-pin') {
    return (
      <div className="min-h-full animate-fade-in">
        <PinPad title="Повторите код-пароль" subtitle="Введите код ещё раз" pinLength={4} error={pinError} onComplete={handlePinConfirm} disabled={loading} />
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-full px-6 animate-scale-in">
      ...
      <p className="text-on-surface-variant text-center text-sm">Добро пожаловать в UnitBank</p>
    </div>
  );
}
